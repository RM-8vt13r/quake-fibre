...
def test_fibre_propagation():
    ...
    try:
        ...
        raise AssertionError("Channel should raise an error when accessing unset path_coordinates, path_lengths, path_positions, and section_coordinates, but didn't")
...
def test_fibre_SOP_rotation_setter():
    channel = Fibre(parameters)
...
def test_fibre_initialisation():
    ...
    assert np.allclose(np.conj(channel.section_SOP_rotation).swapaxes(-2, -1) @ channel.section_SOP_rotation, np.eye(2)), f"Fibre PSP rotation matrices are not unitary"
...
def test_fibre_path():
    channel = Fibre(parameters_geographic)
...
def test_fibre_dict():
    ...
    channel_copy = Fibre.from_dict(fibre_dict)
