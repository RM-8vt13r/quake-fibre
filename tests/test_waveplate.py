...
def test_waveplate_propagation():
    ...
    assert np.allclose(propagated_signal.power_W, signal.power_W), f"Waveplate model did not retain signal energy"
    ...
    assert not np.allclose(propagated_signal.samples_t, propagated_signal_earthquake.samples_t), f"Propagated signal before and after the earthquake were the same, but should be different"


# def test_waveplate_PSP_setters():
#     channel = Waveplate(parameters)
#
