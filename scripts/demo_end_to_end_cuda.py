"""
A script demonstrating the entire end-to-end chain, propagating a continuous wave through a fibre during an earthquake (GPU version).
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument("--GPU", help = "Index of the GPU to use", type = int, required = True)
    ...
    with cp.cuda.Device(arguments.GPU):
        propagated_signals = []
        for _ in arguments.alpha:
            propagated_signals.append(signal.copy())
            propagated_signals[-1].to_device(Device.CUDA)
